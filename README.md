# 👋 Hi there, I’m **Luke Manyamazi**

🎯 **Software Engineer | Full Stack Developer-in-Training | IT Support Technician**  
💡 I’m passionate about solving real problems with code and building projects that matter.

---

## 🚀 About Me

I’m currently building skills in full-stack development while working as an IT Support Technician and studying Computer Engineering. I love learning new technologies and sharing my progress with the dev community.  

I focus on creating practical, real-world solutions — from web apps to utilities — and continuously improving my craft in **security, scalability, performance, and collaboration**.

---

## 🛠️ Tech Skills

**Languages:** JavaScript, Python, C/C#/C++ 
**Frameworks & Tools:** React, Node.js, Express, Django, Git, GitHub  
**Databases:** SQL, PostgresSQL, Firebase
**Cloud:** AWS, Google Cloud, Azure
**Other:** Responsive design, API integration, problem solving

---

## 📂 Featured Projects

Here are some of the projects I’ve built or contributed to — each tells a part of my learning journey:

### 💻 Code Your Future Projects
- **Chat App**
- **Quote App**
- **Q&A App**
- **TV Explorer App**

### 🧠 Real-World & Independent Builds
- **Prepaid Water TopUp - in progress**
- **Dinas Web**
- **Dinas Clock App**
- **Boroma Hills**

### 📊 Advanced / Systems Projects
- **Fitter Board**
- **PIMS – UNI Capstone**

### 🌐 Portfolio
- **Luke Portfolio**
- 🔗 Portfolio: https://lukemanyamazi-portfolio.netlify.app  

---

## 🗂️ Other Notable Repos

- **My-CS50P-Projects** – Python course projects from CS50P
- **Software-Engineering-Course-Assignments**

---

## 👨‍💻 What I’m Learning

- Modern full-stack development workflows
- Business logic behind code
- Cloud deployment basics  
- Writing cleaner, more efficient code  
- AI-assisted development with human judgement

---

## 💬 Let’s Connect

I’m always open to discussion, collaboration, or mentoring.  
📍 Based in Cape Town, South Africa  
🔗 Portfolio: https://lukemanyamazi-portfolio.netlify.app  
📫 Linktree: https://linktr.ee/isirluke

---

> Building in public — one commit at a time 🚀  

